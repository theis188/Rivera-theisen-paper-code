...
if Uini(iDistinctEnz)==0,
    ...
    for i = 1:length(xx)
        ...
        for iModel = 1:EnsembleSize
            if all(~isnan(ModelResults{iModel,1}(:,j,1)))
                ...
                Xvals(i) = ModelResults{1,1}(35,i,1);
            end
        end
    end
    
    plot(log10(xx),tmpFluxes,'Color',Colors(1,:),'LineWidth',2)
    figure
    plot(tmpFluxes,Xvals,'LineWidth',2)
    hold on
    ...
    for n=1:BifPoint,
        %u(62)=(n-1)*200/
        tempFluxes = FLUXES(ModelResults{1,1}(:,n,1), EnsembleKvec(:,1),1,u);
        SelFluxes(:,n) = tempFluxes(enz1);
        PropFlux(n) = sum(tempFluxes(enz1));
        Xvals(n) = ModelResults{1,1}(35,n,1);
    end

    figure
    plot(PropFlux,Xvals,'Color',Colors(2,:));
