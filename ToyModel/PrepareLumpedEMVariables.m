...
%% Create the system of ODEs %%MichaelisMenten Equations
...
%%Mass Action
% TotalParams = sum(Revs + 1)
% 
% KVEC = sym('KVEC', [TotalParams+length(EnzName2) 1]);
% U = sym('U', [length(EnzName)+length(EnzName2) 1]);
% X = sym('X', [size(S,1) 1]);
% 
% [V, ParamInfo, SubstrateConc, ParameterRange, rV] = SetUpRxnRatesMA(S,Sreg, KVEC, X, Vin_index, Vout_index, U,rVnet,Revs, S2a,EnzName,Net.MetabName);
...
for n=1:size(NullMat,2),
    ...
    Expression = solve(CurrentVec'*X - length(find(CurrentVec)), X(tempRemov(1)));
